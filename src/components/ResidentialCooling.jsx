  ...
  return (
    <div ref={ref} className="relative w-full">
      <button type="button" onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between bg-transparent border-b-2 border-white/20 px-0 py-3 text-sm text-left outline-none focus:border-[#8f8cff] transition-colors duration-200 cursor-pointer hover:border-white/40"
      >
        <span className={value && value !== '– Select –' ? 'text-white' : 'text-gray-500'}>{value}</span>
        <ChevronDown size={16} className={`text-[#8f8cff] transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && (
          ...
          {options.map((opt) => (
            <li key={opt} onClick={() => { onChange(opt); setOpen(false); }}
              className={`px-4 py-3 text-sm cursor-pointer transition-colors duration-150 ${
                value === opt ? 'bg-[#8f8cff] text-white font-semibold' : 'text-gray-300 hover:bg-white/10 hover:text-white'
              }`}
            >{opt}</li>
          ))}
        </ul>
      ...
      {steps.map((step, i) => (
        ...
        >
          ...
          <div className="flex flex-col items-center shrink-0">
            <div className="w-10 h-10 rounded-full flex items-center justify-center font-black text-sm border-2 shrink-0 z-10 bg-[#8f8cff]/15 border-[#8f8cff] text-[#8f8cff]">
              {step.num}
            </div>
            {i < steps.length - 1 && (
              <div className="flex-1 w-0.5 my-2 bg-[#8f8cff]/20 min-h-[24px]" />
            )}
          </div>
          {/* Content */}
          <div className="pb-7 pt-1.5">
            <h3 className="text-white font-black text-sm sm:text-base mb-1.5 flex items-center gap-2">
              <step.Icon size={16} className="text-[#8f8cff] shrink-0" />
              {step.title}
            </h3>
  ...
  return (
    <section className="w-full bg-[#1a1a2e] text-white pt-28 pb-20 sm:pt-32 sm:pb-24 lg:pt-36 lg:pb-32 px-4 sm:px-8 lg:px-16 overflow-hidden">
      ...
      <style>{`
        ...
        .split-btn-red::before{content:'';position:absolute;inset:0;right:50%;background:#c3252e;transition:transform 0.38s cubic-bezier(0.77,0,0.175,1);z-index:0;}
        .split-btn-red::after{content:'';position:absolute;inset:0;left:50%;background:#c3252e;transition:transform 0.38s cubic-bezier(0.77,0,0.175,1);z-index:0;}
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-20 lg:gap-28">
        ...
        >
          <span className="text-[#8f8cff] text-xs font-bold uppercase tracking-widest block mb-4">Residential Solutions</span>
          <h1 id="metric-rescooling-heading" className="font-black leading-tight text-4xl sm:text-5xl lg:text-6xl mb-5">
            <span className="text-[#c3252e]">Home Cooling </span>
            <span className="text-[#8f8cff]">& Air </span><br />
        ...
        <div ref={servicesRef}>
          ...
          >
            <span className="text-[#8f8cff] text-xs font-bold uppercase tracking-widest block mb-3">What We Do</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {SERVICES.map(({ Icon, title, desc }, i) => (
              <div
                key={title}
                className="flex flex-col gap-4 p-7 rounded-2xl bg-white/5 border border-white/8 hover:border-[#8f8cff]/40 hover:bg-white/8 transition-all duration-300 group"
                style={{
                  opacity: servicesInView ? 1 : 0,
              ...
              >
                <div className="w-12 h-12 rounded-xl bg-[#8f8cff]/15 border border-[#8f8cff]/25 flex items-center justify-center group-hover:bg-[#8f8cff]/25 transition-colors duration-300">
                  <Icon size={22} className="text-[#8f8cff]" />
                </div>
        ...
        <div ref={centralRef}>
          ...
          >
            <span className="inline-flex items-center gap-2 text-[#8f8cff] text-xs font-bold uppercase tracking-widest mb-3">
              <span className="w-6 h-px bg-[#8f8cff]" />How It Works<span className="w-6 h-px bg-[#8f8cff]" />
            </span>
          ...
          {/* Thermostat note */}
          <div
            className="mt-8 mx-auto max-w-lg text-center p-4 rounded-xl bg-[#8f8cff]/10 border border-[#8f8cff]/25"
            style={{ opacity: centralInView ? 1 : 0, transition: 'opacity 0.7s ease 0.6s' }}
          >
            <p className="text-[#8f8cff] text-xs font-bold uppercase tracking-wider mb-1">Pro Tip</p>
        ...
        <div ref={ductlessRef}>
          ...
          >
            <span className="inline-flex items-center gap-2 text-[#8f8cff] text-xs font-bold uppercase tracking-widest mb-3">
              <span className="w-6 h-px bg-[#8f8cff]" />No Ducts? No Problem<span className="w-6 h-px bg-[#8f8cff]" />
            </span>
            ...
            <div className="flex flex-col gap-0">
              {DUCTLESS_POINTS.map((point, i) => (
                ...
                >
                  {/* Timeline */}
                  <div className="flex flex-col items-center shrink-0">
                    <div className="w-10 h-10 rounded-full bg-[#8f8cff]/15 border border-[#8f8cff]/30 flex items-center justify-center shrink-0 z-10">
                      <point.Icon size={16} className="text-[#8f8cff]" />
                    </div>
                    {i < DUCTLESS_POINTS.length - 1 && (
                      <div className="flex-1 w-0.5 my-2 bg-[#8f8cff]/20 min-h-[20px]" />
                    )}
                  </div>
                  {/* Text */}
        ...
        <div ref={formRef} id="cooling-quote-form">
          ...
          >
            <span className="text-[#8f8cff] text-xs font-bold uppercase tracking-widest block mb-3">Ready to Start?</span>
