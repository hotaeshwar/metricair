    ...
    <div className="w-full bg-[#1a1a2e] text-white pt-24 pb-16 sm:pt-32 lg:pt-36 overflow-hidden">
      ...
      >
        ...
        <h1 className="font-black leading-tight text-3xl sm:text-5xl lg:text-6xl mb-6">
          <span className="text-[#3b82f6]">Commercial</span>{' '}
          <span className="text-[#e94560]">HVAC</span>{' '}
      ...
      <section ref={gridRef} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-20 sm:mb-28">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {SUB_SERVICES.map((srv, idx) => (
            <div 
              key={srv.title} 
              className="p-6 rounded-2xl bg-white/5 border border-white/10 hover:border-[#3b82f6]/40 hover:bg-white/[0.08] flex flex-col gap-4 transition-all duration-300 group overflow-hidden"
              style={{
                opacity: gridInView ? 1 : 0,
            ...
            >
              ...
              <div className="flex-1 flex flex-col justify-between">
                  ...
                  <div className="flex flex-col gap-1.5 mb-6">
                    {srv.features.map((feat) => (
                      <span key={feat} className="text-[10px] sm:text-xs font-semibold text-gray-300 flex items-center gap-1.5">
                        <span className="text-[#3b82f6]">✓</span> {feat}
                      </span>
                    ))}
                  </div>
                </div>
                <Link 
                  to={srv.href} 
                  className="w-full block py-2.5 rounded-lg bg-[#3b82f6] hover:bg-[#2563eb] transition-colors text-center text-xs font-bold uppercase tracking-wider text-white"
                >
                  View Details
                </Link>
              </div>
      ...
      >
        <div className="rounded-3xl bg-white/5 border border-white/10 p-6 sm:p-10 lg:p-12">
          <div className="text-center mb-8 sm:mb-10">
            <span className="text-[#3b82f6] text-xs font-bold uppercase tracking-widest block mb-2">Commercial Proposals</span>
